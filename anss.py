...
st.markdown("""
    ...
    /* أمر برمجي لإخفاء أزرار التحكم الجانبية في جميع خانات الأرقام */
    ...
    /* تنسيق خانة الإدخال لتكون مربعة ونظيفة */
    ...
    .label-text { font-size: 1.1rem; font-weight: bold; color: #00d1ff; }
    .steps-card { 
    ...
    .stButton>button {
        ...
        color: white; font-weight: bold; border: none; width: 100%; height: 3.5em; border-radius: 8px;
    }
    </style>
    """, unsafe_allow_html=True)

# --- 2. المحرك الرياضي (حل القيود والحذف) ---
class HusseinSolver:
    @staticmethod
    ...
    def solve_with_steps(c1, c2, constraints):
        ...
        if HusseinSolver.is_feasible((x, y), constraints):
            ...
            report += f"  Solving the system: \n  {a1}X1 + {b1}X2 = {r1}\n  {a2}X1 + {b2}X2 = {r2}\n"
            report += f"  Result: Feasible corner point at ({x:.2f}, {y:.2f})\n\n"
            return (x, y), report
        return None, ""
...
with col_in:
    ...
    # اختيار عدد القيود (2 أو 3 أو 4)
    st.markdown('<p class="label-text">📏 عدد القيود (Constraints)</p>', unsafe_allow_html=True)
    num_c = st.selectbox("اختر عدد القيود من القائمة:", [2, 3, 4], index=1)
...
if run_btn:
    try:
        ...
        report += "STEP 1: FINDING AXIS INTERCEPTS\n"
        ...
        unique_corners = []
        seen = set()
        report += "="*45 + "\nSTEP 3: CALCULATING Z FOR ALL CORNERS\n"
        for p in corner_points:
            ...
            if rnd not in seen:
                ...
                report += f" Corner ({p[0]:.2f}, {p[1]:.2f}) -> Z = {z_val:.2f}\n"

        if not unique_corners:
            st.error("Infeasible! (لا توجد منطقة حل تحقق القيود)")
        else:
            ...
            with col_res:
                ...
                ax.tick_params(colors='white'); ax.grid(color='#30363d', alpha=0.4)
        ...
        st.subheader("📄 تقرير التحليل الرياضي")
