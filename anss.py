...
st.set_page_config(
    page_title="Hussein Math Solver",
    page_icon="📱",
    layout="wide"
)

# تصميم مخصص لمحاكاة واجهة التطبيق المظلمة والمرتبة
st.markdown("""
    <style>
    .stApp { background-color: #0d1117; color: #ffffff; }
    
    /* إخفاء أزرار الزائد والناقص نهائياً من خانات الأرقام */
    button.step-up, button.step-down { display: none !important; }
    input[type=number]::-webkit-inner-spin-button, 
    input[type=number]::-webkit-outer-spin-button { -webkit-appearance: none; margin: 0; }
    input[type=number] { -moz-appearance: textfield; }

    /* تنسيق الخانات الرقمية لتكون واضحة وبسيطة */
    ...
    /* تنسيق صندوق الحل المفصل ليكون عمودياً ومنظماً */
    .steps-container { 
        ...
        color: #c9d1d9; 
        line-height: 1.6;
        white-space: pre; /* يحافظ على الفواصل والأسطر كما هي */
        overflow-x: auto;
        font-size: 14px;
    }
...
# --- 2. المحرك الرياضي (حل المعادلات والحذف) ---
class HusseinSolver:
    @staticmethod
...
# --- 3. واجهة الإدخال والنتائج ---
st.title("🛡️ Hussein Solver Pro")
st.write("نظام التحليل الرياضي المفصل - نسخة الجوال")
...
if run_btn:
    try:
        # --- توليد التقرير الرياضي المفصل (الترتيب العمودي) ---
        ...
        for coeffs, op, rhs, idx in constraints_list:
            ...
            if coeffs[0] != 0:
                ...
                report += f"   => X1 = {p[0]:.2f}. Point: ({p[0]:.2f}, 0)\n"
            ...
            if coeffs[1] != 0:
                ...
                report += f"   => X2 = {p[1]:.2f}. Point: (0, {p[1]:.2f})\n"
                if HusseinSolver.is_feasible(p, constraints_list):
                    corner_points.append(p)
                    report += "   [Status: Feasible Corner Point]\n"
                else:
        ...
        st.write("---")
        # بطاقات النتائج المختصرة
        res_col1, res_col2 = st.columns(2)
        res_col1.metric("Optimal Z", f"{best[1]:.2f}")
        res_col2.metric("Point", f"({best[0][0]:.2f}, {best[0][1]:.2f})")

        # عرض التقرير الرياضي (الأهم)
